...
_SYSTEM = (
    "你是知識庫測試集出題者。根據使用者提供的『單一頁面原文』，產生一個可以"
    "『只靠這頁內容』回答的繁體中文問題，以及精簡的標準答案。要求：\n"
    "1. 問題必須具體、提到頁面中的關鍵名詞或數據，像真實使用者會問的問題。\n"
    "2. 問題本身不可出現『這頁/本頁/以上/根據上文』等指涉，要能獨立成立。\n"
    "3. 答案精簡（一兩句），且確實出自該頁原文。\n"
    "4. 只輸出 JSON：{\"question\": \"...\", \"answer\": \"...\"}，不要其他文字。"
)
...
def _iter_golden(service: DocumentService, doc_ids: list[str] | None, per_doc: int, min_chars: int):
    ...
    for doc in docs:
        ...
        for page in chosen:
            ...
            result = chat_json(_SYSTEM, excerpt[:4000], max_tokens=512)
            if not result or not result.get("question"):
                print(f"  [skip] {doc.file_name} p{page.page_number}: no usable question")
                continue
            yield {
                "id": f"{doc.document_id}-p{page.page_number}",
                "query": str(result["question"]).strip(),
            ...
            print(f"  [ok]   {doc.file_name} p{page.page_number}: {str(result['question'])[:50]}")
...
def main() -> None:
    ...
    items = list(_iter_golden(service, doc_ids, args.per_doc, args.min_chars))
    if not items:
